...
static int next_free_slot(sel4utils_process_t *process, cspacepath_t *dest)
{
    if (process->cspace_next_free >= (BIT(process->cspace_size))) {
        ZF_LOGF("Can't allocate slot, cspace is full.\n");
        return -1;
    }
    ...
    seL4_CPtr sel4utils_copy_cap_to_process(sel4utils_process_t *process, vka_t *vka, seL4_CPtr cap)
{
    ...
    vka_cspace_make_path(vka, cap, &src_path);
    return sel4utils_copy_path_to_process(process, src_path);
}
...
int sel4utils_osm_configure_process_custom(sel4utils_process_t *process, vka_t *vka,
                                       ...
                                       sel4utils_process_config_t config)
{
    ...
    memcpy(&process->pd, target_vspace_root_page_dir, sizeof(vka_object_t));
    ...
    // /* (XXX): Here used passed on vpsace */
    ...
    // (XXX): This needs to be updated, so that it takes in a array of MOs, or an MO allocator.
